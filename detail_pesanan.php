...
$stmt = mysqli_prepare($koneksi, "SELECT orders.*, users.nama_lengkap FROM orders JOIN users ON orders.user_id = users.id WHERE orders.id = ?");
...
// FIXED: Redirect if order not found
if (!$order) {
    header("Location: admin.php");
    exit;
}
 
// FIXED: Sanitize all output variables up front
...
$safe_total      = number_format($order['total_harga'], 0, ',', '.');
...
<head>
    <meta charset="UTF-8">
    <!-- FIXED: Added missing viewport meta tag -->
    ...
    <style>
        ...
        /* FIXED: Consolidated table styles into one block */
        .table-detail {
            width: 100%;
        ...
        /* FIXED: Moved print styles into <head> where they belong */
        @media print {
...
<body>
    ...
    <div class="container">
        ...
        <div class="info-pesanan">
            <!-- FIXED: All variables sanitized before output -->
        ...
        <div class="table-wrapper">
            <table class="table-detail">
                ...
                <tbody>
                    <?php
                    // FIXED: Prepared statement for order details query
                    $stmt2 = mysqli_prepare($koneksi, "SELECT order_details.*, menus.nama_makanan FROM order_details JOIN menus ON order_details.menu_id = menus.id WHERE order_details.order_id = ?");
                    ...
                    if (!$result2) {
                        ...
                        while ($d = mysqli_fetch_assoc($result2)) {
                            $nama_menu  = htmlspecialchars($d['nama_makanan']);
                            $jumlah     = htmlspecialchars($d['jumlah']);
                            $harga_sat  = number_format($d['harga_satuan'], 0, ',', '.');
